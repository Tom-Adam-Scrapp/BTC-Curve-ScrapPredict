...
class visual_content:
    def __init__(self):
        ...
        self.frame_window_bottom = tk.Frame(self.app, bg="white", borderwidth=2)
        ...
        self.button_quit = tk.Button(self.frame_window_top_left, text="Close app", command=self.close_window)
        ...
        self.label_bottom = tk.Label(self.frame_window_bottom, text = "Il faut attendre un peu le temps que le webscrapping recupere l'information")
    ...
    def organise_widgets(self):
        ...
        self.app.title("Bitcoin value displayer")
...
# value to display
def usefull_value(array_values, index):
    global std_btc, last_value, avg_10_last, min_btc, max_btc
    std_btc = np.std(np.array(array_values))

    last_value = array_values[index]
    min_btc = np.min(np.array(array_values))

    max_btc = np.max(np.array(array_values))
    if len(array_values) > 10:
        last_10 = array_values[-10]
...
def webscrap_to_graph_update():
    ...
    try:
        ...
        global dot_counter, launch1, time_btw_dot, \
            np_list_value_bitcoin_array, dot_counter, ax
        ...
        ax.plot(x[:dot_counter+1], y[:dot_counter+1], c="black")  # creates new line

        # Add below threshold markers
        if len(x) > 10:
            below_threshold = x > x[dot_counter-1]
        ...
        application.label_bottom.pack_forget()
        usefull_value(y, dot_counter+1)
        update_value_appearance()
    except:
        application.label_bottom.pack(side='left', fill='both', anchor='w')
        update_graph = False
        button_graph_string.set("Launch graph")
        print("too early")
